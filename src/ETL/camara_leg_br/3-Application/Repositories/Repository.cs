...
namespace DeOlho.ETL.camara_leg_br.Infrastructure.Repositories
{
    public class Repository<T> : IRepository<T> where T : Entity
    {
        ...
        public IUnitOfWork UnitOfWork { get => _deOlhoDbContext; }
        ...
        {
            return _deOlhoDbContext.Set<T>().ToList();
        }

        public async Task<IEnumerable<T>> ListAllAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            return await _deOlhoDbContext.Set<T>().ToListAsync();
        }
