...
namespace DeOlho.ETL.dadosabertos_camara_leg_br.Api.Services
{
    public class ETLService : IETLService
    {
        ...
        {
            await execute(async (c, t) => await executeLegislatura(c, t));
        }
        private async Task executeLegislatura(IDbConnection destinationDbConnection, IDbTransaction destinationDbTransaction)
        {
            var load = await new Process()
